...
function getVersion() {
    ...
    const numOfVersions = (pkg.version || "0.0.1").split(".").map(x => +x);
    const mainVersion = numOfVersions.slice(0, 3).join(".");
    const buildVersion = parseInt(new Date().toJSON().replace(/\D/g, "").substring(2, 12), 10);
    const version = mainVersion + (isDevMode ? ".build" + buildVersion : "");
    const name = pkg.name.toUpperCase().replace(/-/g, " ");

    return {
        banner: getBanner(name, isDevMode ? version : "ver. " + version),
        name: name,
        version: version,
